...
export class MessageGuard<T extends string, P> {
	...
	type<TOut extends T>(t: TOut) {
		return new MessageGuard<TOut, P>(t)
	}

	payload<POut>() {
		return new MessageGuard<T, POut>(this.T)
	}
...
export const message = new MessageGuard<string, unknown>('')
